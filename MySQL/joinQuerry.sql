...
INNER JOIN offices o -- JOIN == INNER JOIN
ON e.officecode = o.officeCode ORDER BY employeeNumber;
...
SELECT employeeNumber, firstName, city, country FROM
employees JOIN offices USING (officeCode);

# Design a query to display custNo, custName, SalesRepName of the customer

SELECT customerNumber, customerName, CONCAT(FirstName, ' ', lastName) AS 'Sales Rep', email
...
SELECT orders.`status`, SUM(quantityOrdered*priceEach) total
FROM orders
...
SELECT e.employeeNumber, CONCAT(e.firstName,' ', e.lastName) 'Full Name',

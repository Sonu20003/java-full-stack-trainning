...
SELECT * FROM customers;
SELECT customerName, phone, addressLine1 FROM customers WHERE customerNumber >= 200;
...
SELECT * FROM orderdetails 
WHERE quantityOrdered >= 50 OR priceEach > 100;	
...
SELECT * FROM employees WHERE officecode IN(1,2,7) ORDER BY officecode;

SELECT * FROM employees WHERE LastName IN('Young', 'Smith');

SELECT * FROM employees WHERE LastName NOT IN('Young', 'Smith');
...
FROM products
ORDER BY 4 DESC; -- 4 is column location
-- Multi Level Sorting
...
SELECT orderNumber, orderlinenumber, quantityOrdered * priceEach AS 'total_price' FROM orderdetails
...
SELECT DISTINCT contactLastName, state, city FROM customers WHERE state IS NOT NULL
ORDER BY 1,2,3;
...
SELECT * FROM customers ORDER BY customerNumber LIMIT 5 OFFSET 4;
...
SELECT sum(quantityInStock) AS 'Total sum' FROM products;
SELECT productLine, quantityInStock FROM products;
SELECT productLine, COUNT(quantityInStock) 'Quantity in stock' FROM products GROUP BY productLine;
 

-- Query to display total no. of customers from each city

-- waq TO COUNT NUMBER OF sales reps

SELECT jobtitle, COUNT(*) AS 'Count' FROM employees GROUP BY jobTitle;
SELECT lastName, COUNT(*) 'Count' FROM employees GROUP BY lastName HAVING COUNT(lastName) > 2;
...
SELECT YEAR(orderdate) AS Year_, COUNT( orderNumber) FROM 
	orders GROUP BY Year_ ORDER BY Year_ desc;
...
SELECT SUBSTR('JDBC Programming',6,7); -- Returns part of a string - specific pos,no. of characters



SELECT LOWER(customerName) customer, customerName, city FROM customers;
SELECT UPPER(customerName) customer, customerName, city FROM customers;
SELECT UPPER(customerName) customer, customerName, MID(city,1,3) Shortcity, city FROM customers;
...
SELECT orderNumber, requiredDate, shippedDate, DATEDIFF(requiredDate, shippedDate) DaysLeft FROM orders
...
SELECT CONVERT('-1',UNSIGNED INTEGER), CONVERT('-100',SIGNED INTEGER);

SELECT CONVERT(NOW(),DATE), CONVERT(NOW(),TIME);